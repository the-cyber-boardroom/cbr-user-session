...
class Routes__Session(Fast_API_Routes):
    ...
    def session_exists(self, session_id):
        return self.user_session_utils.session_exists(session_id)                       # BUG: this is taking about 600ms when it should be max 50ms

    def session_exists__with_trace(self, session_id:str, bigger_than:float=0.020):
        config = dict(trace_capture_start_with  = ['botocore'],
        ...
        with Trace_Call(config=trace_call_config) as trace_call:                # capture all calls that match the config that are executed inside here
